...
#define index(x,y) strchr(x,y)
	...
	*/
	...
	int handleGetMethod(struct ParsedRequest * parse, const char *buf,
		char *saveptr , char *tmp_buf)
	{
		char *full_addr;
		char *currentHeader;
	...
	*/
	...
	int handleConnectMethod(struct ParsedRequest * parse, const char *buf,
		char *saveptr, char *tmp_buf)
	{
		char *full_addr;
		char *currentHeader;
		...
		char * protocol_version = full_addr + strlen(full_addr) + 1;
		if (protocol_version == NULL) {
			debug("invalid request line, missing protocol and version\n");
			return -1;
		}

		parse->protocol = strtok_r(NULL, "/", &protocol_version);
		if (parse->protocol == NULL) {
			debug("invalid request line, missing protocal\n");
			return -1;
		}
		parse->version = protocol_version;
		if (parse->version == NULL) {
			debug("invalid request line, missing version\n");
			return -1;
		}
		...
		int leng_host_str = strlen(full_addr);
		char * temp_full_addr = malloc(leng_host_str);
		memcpy(temp_full_addr, full_addr, leng_host_str);
		temp_full_addr[leng_host_str] = '\0';

		parse->host = strtok_r(NULL, ":", &temp_full_addr);
		if (parse->host == NULL) {
			free(temp_full_addr);
			debug("invalid request line, missing path\n");
			return -1;
		}
		parse->port = temp_full_addr;
		if (parse->port == NULL) {
			free(temp_full_addr);
	...
	*/
			...
			int buflen)
	{
		...
		{
			res =  handleConnectMethod(parse, buf, saveptr, tmp_buf);
		}
